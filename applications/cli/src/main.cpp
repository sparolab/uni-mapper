...
int main(int argc, char** argv) {
  ...
  auto opened = runtime.Open({argv[1], "batch", std::nullopt});
  if (!opened) {
  ...
  if (!progress_subscription) {
    WriteError(progress_subscription.GetError().Message());
    return 1;
  }
  auto submitted = runtime.SubmitRunAll();
  if (!submitted) {
    WriteError(submitted.GetError().Message());
    return 1;
  }
  auto completed = runtime.Wait(submitted.Value());
  if (!completed) {
    WriteError(completed.GetError().Message());
    return 1;
  }
