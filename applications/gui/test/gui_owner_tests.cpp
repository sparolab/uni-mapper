...
class ThrowingGui final : public open_lmm::GuiPlugin {
 public:
  enum class Failure { kStandard, kUnknown };

  explicit ThrowingGui(Failure failure) : failure_(failure) {}

  open_lmm::Result<void> Start(open_lmm::GuiServices) override {
    if (failure_ == Failure::kStandard) {
      throw std::runtime_error("fixture start failure");
    }
    throw 7;
  }
...
struct EventCallbackSlot {
  std::mutex mutex;
  std::function<void(const open_lmm::ExecutionEvent&)> callback;
};

class SubscriptionRaceGui final : public open_lmm::GuiPlugin {
 public:
  open_lmm::Result<void> Start(open_lmm::GuiServices services) override {
    queue_ = std::make_shared<open_lmm::GuiEventQueue>(32);
    weak_queue_ = queue_;
    if (!services.subscribe_events) {
      return open_lmm::Result<void>::Failure(
          open_lmm::Error::InvalidArgument("subscription service is missing"));
    }
    auto subscribed = services.subscribe_events(
        [weak_queue = weak_queue_](const open_lmm::ExecutionEvent& event) {
          if (auto queue = weak_queue.lock()) queue->Push(event);
        });
    if (!subscribed) return open_lmm::Result<void>::Failure(subscribed.GetError());
    subscription_ = std::move(subscribed).Value();
    open_ = true;
    return open_lmm::Result<void>::Ok();
  }
  ...
  [[nodiscard]] bool QueueExpired() const { return weak_queue_.expired(); }

 private:
  open_lmm::ExecutionEventSubscription subscription_;
  std::shared_ptr<open_lmm::GuiEventQueue> queue_;
  std::weak_ptr<open_lmm::GuiEventQueue> weak_queue_;
  bool open_ = false;
};

void TestEventSubscriptionStopRace() {
  auto slot = std::make_shared<EventCallbackSlot>();
  open_lmm::GuiServices services;
  services.subscribe_events =
      [slot](std::function<void(const open_lmm::ExecutionEvent&)> callback) {
        {
          std::lock_guard lock(slot->mutex);
          slot->callback = std::move(callback);
        }
        return open_lmm::Result<open_lmm::ExecutionEventSubscription>::Ok(
            open_lmm::ExecutionEventSubscription([slot] {
              std::lock_guard lock(slot->mutex);
              slot->callback = {};
            }));
      };

  auto plugin = std::make_shared<SubscriptionRaceGui>();
  open_lmm::GuiPluginHost host(plugin);
  Require(host.Start(std::move(services)).IsOk(),
          "subscription-race fixture starts");

  std::function<void(const open_lmm::ExecutionEvent&)> stale_callback;
  {
    std::lock_guard lock(slot->mutex);
    stale_callback = slot->callback;
  }
  Require(static_cast<bool>(stale_callback),
          "subscription-race fixture publishes its callback");

  std::atomic<bool> stop_emitter{false};
  std::atomic<unsigned> emitted{0};
  std::thread emitter([&] {
    open_lmm::ExecutionEvent event;
    event.type = open_lmm::EventType::kProgressUpdated;
    while (!stop_emitter.load(std::memory_order_acquire)) {
      std::function<void(const open_lmm::ExecutionEvent&)> callback;
      {
        std::lock_guard lock(slot->mutex);
  ...
  while (emitted.load(std::memory_order_acquire) < 100 &&
         std::chrono::steady_clock::now() < deadline) {
    std::this_thread::yield();
  }
  Require(emitted.load(std::memory_order_acquire) >= 100,
          "subscription callback races with an active emitter");

  host.Stop();
  stale_callback({});
  stop_emitter.store(true, std::memory_order_release);
  emitter.join();

  bool callback_cleared = false;
  {
    std::lock_guard lock(slot->mutex);
    callback_cleared = !slot->callback;
  }
  Require(callback_cleared && plugin->QueueExpired() && !host.IsOpen(),
          "stop resets subscription before queue destruction and stale callbacks are harmless");
}
...
void TestGuiModelAndConfigEditor() {
  ...
  {
    open_lmm::GuiPluginHost host(plugin);
    Require(host.Start({}).IsOk(), "plugin host starts");
    const auto duplicate_start = host.Start({});
    Require(!duplicate_start &&
                duplicate_start.GetError().code ==
                    open_lmm::Error::Code::kInvalidArgument,
            "plugin host rejects a duplicate start");
    host.Stop();
    host.Stop();
  }
  Require(plugin->requested_stop && plugin->joined, "plugin host stops and joins");

  open_lmm::GuiPluginHost null_host(std::shared_ptr<open_lmm::GuiPlugin>{});
  const auto null_start = null_host.Start({});
  Require(!null_start &&
              null_start.GetError().code ==
                  open_lmm::Error::Code::kInvalidArgument,
          "plugin host rejects a null plugin");

  for (const auto failure : {ThrowingGui::Failure::kStandard,
                             ThrowingGui::Failure::kUnknown}) {
    open_lmm::GuiPluginHost host(std::make_shared<ThrowingGui>(failure));
    const auto started = host.Start({});
    Require(!started &&
                started.GetError().code ==
                    open_lmm::Error::Code::kPluginLoadFailed &&
                !host.IsOpen(),
            "plugin host maps every Start exception without publishing open");
    if (failure == ThrowingGui::Failure::kStandard) {
      Require(started.GetError().Message().find("fixture start failure") !=
                  std::string::npos,
              "plugin host preserves the standard Start exception payload");
    } else {
      Require(started.GetError().Message().find("unknown exception") !=
                  std::string::npos,
              "plugin host maps non-standard Start exceptions explicitly");
    }
  }
}
...
void TestSessionFreeBridgeAndModelReplacement() {
  ...
  auto opened = runtime->Open({root / "config", "gui", std::nullopt});
