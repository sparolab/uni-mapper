 ...
 public:
  ...
  Result<void> RunNode(NodeId, std::optional<AgentId>) {
    ...
    if (non_cooperative_node) {
      release_non_cooperative.Wait("non-cooperative fixture release");
      node_exited.store(true, std::memory_order_release);
      return Result<void>::Ok();
    }
    if (block_node_until_cancel) {
      allow_cancellation_check.Wait("cancellation check release");
      if (!cancellation || !cancellation->IsCancellationRequested()) {
        return Result<void>::Failure(
            Error::InvalidArgument("cancellation fixture released before cancel"));
      }
  ...
  Result<void> RunStage(StageId stage) {
    ...
    if (stage == StageId::kAlignment && request_alignment_feedback) {
      ...
      if (hold_mutex_during_feedback) {
        runner_lock.lock();
        feedback_notification_gate.ArriveAndWait(
            "feedback notification release");
      }
  ...
  test::ManualResetEvent node_entered;
  test::ManualResetEvent allow_cancellation_check;
  bool non_cooperative_node = false;
  test::ManualResetEvent release_non_cooperative;
  ...
  test::PhaseGate feedback_notification_gate;
...
class AlignmentFeedbackObserver {
 public:
  using Predicate = std::function<bool(const AlignmentFeedbackSnapshot&)>;

  explicit AlignmentFeedbackObserver(PipelineController& controller)
      : controller_(controller),
        predicate_([](const AlignmentFeedbackSnapshot&) { return true; }),
        subscription_(controller.SubscribeEvents(
            [this](const ExecutionEvent& event) {
              if (event.type == EventType::kAlignmentFeedbackRequested) {
                const auto snapshot = controller_.GetAlignmentFeedbackSnapshot();
                if (!snapshot) return;
                {
                  std::lock_guard lock(mutex_);
                  if (!predicate_(*snapshot)) return;
                  matched_ = snapshot;
                }
                changed_.Signal();
              }
  ...
  AlignmentFeedbackSnapshot Wait() const {
    changed_.Wait("alignment feedback publication");
    std::lock_guard lock(mutex_);
    if (!matched_) throw std::runtime_error("feedback signal without snapshot");
    return *matched_;
  }
 ...
 private:
  ...
  test::ManualResetEvent changed_;
  Predicate predicate_;
  std::optional<AlignmentFeedbackSnapshot> matched_;
...
void TestConcurrentSubmissionsAreSerialized() {
  ...
  runner->node_entered.Wait("accepted concurrent node entered");
  Check(controller.Cancel(accepted_id).IsOk(),
        "accepted concurrent submission remains cancellable");
  runner->allow_cancellation_check.Signal();
...
void TestNodeCancellationRollsBackArtifacts() {
  ...
  runner->node_entered.Reset();
  runner->allow_cancellation_check.Reset();
  runner->block_node_until_cancel = true;
  auto rerun = controller.SubmitNode(NodeId::kDataLoad, Id("A"));
  Check(rerun.IsOk(), "cancellable node submitted");
  runner->node_entered.Wait("cancellable node entered");
...
void TestCancellationTelemetryAndJoinOrdering() {
  ...
  runner->release_non_cooperative.Signal();
  PipelineController controller(runner);
  ...
  runner->release_non_cooperative.Reset();
  ...
  runner->node_entered.Wait("non-cooperative node entered");
...
void TestAlignmentFeedbackAcceptAndStaleResponse() {
  ...
  AlignmentFeedbackObserver feedback(controller);
  auto job = controller.SubmitStage(StageId::kAlignment);
  Check(job.IsOk(), "interactive alignment submitted");
  feedback.Wait();
  auto request = controller.GetAlignmentFeedbackSnapshot();
  Check(request.has_value(), "interactive feedback snapshot published");
...
void TestSnapshotDuringFeedbackNotificationDoesNotDeadlock() {
  ...
  runner->feedback_notification_gate.WaitUntilEntered(
      "runner feedback lock inversion window");
  ...
  runner->feedback_notification_gate.Release();
  feedback.Wait();
  auto request = controller.GetAlignmentFeedbackSnapshot();
...
void TestControllerCoordinatorFullFallbackIntegration() {
  ...
  feedback.Wait();
  auto request = controller.GetAlignmentFeedbackSnapshot();
  Check(request.has_value(), "coordinator feedback snapshot published");
  const uint64_t kiss_request = request->proposal.request_id;
  const uint64_t kiss_revision = request->session_revision;
  feedback.Expect([kiss_revision](const AlignmentFeedbackSnapshot& snapshot) {
    return snapshot.session_revision != kiss_revision &&
  ...
  Check(request && request->session_revision != kiss_revision &&
            request->proposal.method == AlignmentMethod::kDescriptor,
        "Descriptor feedback revision published");
...
void TestControllerKeepsWaitingAfterRecoverableAttempt() {
  ...
  feedback.Wait();
  auto request = controller.GetAlignmentFeedbackSnapshot();
  Check(request.has_value(), "recoverable feedback snapshot published");
  const auto session_id = request->proposal.request_id;
  const auto initial_revision = request->session_revision;
  feedback.Expect(
      [initial_revision](const AlignmentFeedbackSnapshot& snapshot) {
        return snapshot.session_revision != initial_revision &&
  ...
  feedback.Wait();
  request = controller.GetAlignmentFeedbackSnapshot();
  Check(request && request->session_revision != initial_revision &&
            request->attempt_status.state ==
                AlignmentAttemptState::kFailedRecoverable,
        "recoverable failure feedback revision published");
  ...
  feedback.Expect([failed_revision](const AlignmentFeedbackSnapshot& snapshot) {
    return snapshot.session_revision != failed_revision &&
           snapshot.proposal.method == AlignmentMethod::kDescriptor;
  });
  Check(controller.RespondToAlignment(
  ...
  feedback.Wait();
  request = controller.GetAlignmentFeedbackSnapshot();
  Check(request && request->session_revision != failed_revision &&
            request->proposal.method == AlignmentMethod::kDescriptor,
        "retry feedback revision published");
  Check(controller.RespondToAlignment(
...
void TestControllerCoordinatorFatalProposerIntegration() {
  ...
  feedback.Wait();
  auto request = controller.GetAlignmentFeedbackSnapshot();
  Check(request.has_value(), "fatal proposer feedback snapshot published");
  Check(controller.RespondToAlignment(
            job.Value(), {request->proposal.request_id,
