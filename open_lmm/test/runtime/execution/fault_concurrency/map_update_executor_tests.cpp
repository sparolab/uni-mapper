...
struct OperationProbe {
  explicit OperationProbe(std::size_t parallel_entries = 1)
      : expected_parallel_entries(parallel_entries),
        remover_arrivals(parallel_entries),
        heavy_callback_arrivals(parallel_entries) {}
  ...
  test::CountdownEvent remover_arrivals;
  test::CountdownEvent heavy_callback_arrivals;
  test::ManualResetEvent first_failure_reached;
  test::ManualResetEvent second_task_blocked;
  test::ManualResetEvent release_second_task;
  test::ManualResetEvent cancellation_wait_entered;
  test::ManualResetEvent cancellation_requested;
};
...
class FixtureRemover final : public DynamicRemoverBase {
 public:
  ...
  Result<PointCloud::Ptr> ProcessStreaming(
    ...
    probe_->remover_arrivals.Arrive();
    probe_->remover_arrivals.Wait("parallel MapUpdate remover arrivals");
    ...
    if (probe_->behavior == RemoverBehavior::kDrainFirstFailure) {
      if (context.agent.id == Id("A")) {
        ...
        probe_->first_failure_reached.Signal();
        return Result<PointCloud::Ptr>::Failure(
            Error::InvalidArgument("first agent failure"));
      }
      probe_->second_blocked.store(true, std::memory_order_release);
      probe_->second_task_blocked.Signal();
      probe_->release_second_task.Wait("release blocked MapUpdate sibling");
    ...
    if (probe_->behavior == RemoverBehavior::kWaitForCancellation) {
      ...
      probe_->cancellation_wait_entered.Signal();
      probe_->cancellation_requested.Wait("MapUpdate cancellation request");
      Check(context.cancellation->IsCancellationRequested(),
            "cancellation release follows the cancellation request");
      return Result<PointCloud::Ptr>::Failure(
    ...
    if (input.heavy_phase_admission) {
      probe_->heavy_callbacks_observed.fetch_add(1,
                                                 std::memory_order_relaxed);
      probe_->heavy_callback_arrivals.Arrive();
      auto phase = input.heavy_phase_admission();
      if (!phase) {
        return Result<PointCloud::Ptr>::Failure(phase.GetError());
      }
      if (probe_->behavior != RemoverBehavior::kDrainFirstFailure) {
        probe_->heavy_callback_arrivals.Wait(
            "parallel MapUpdate heavy callbacks");
      }
      const unsigned active =
...
void TestMidOperationCancellationReleasesAdmissionAndHeavyPhase() {
  ...
  probe->cancellation_wait_entered.Wait(
  ...
  probe->cancellation_requested.Signal();
  execution.join();
...
void TestParallelFailureDrainsEverySubmittedTask() {
  ...
  auto probe = std::make_shared<OperationProbe>(2);
  probe->governor = governor;
  ...
  test::ManualResetEvent execution_returned;
  std::thread execution([&] {
    result = executor.Execute(Context(state, governor, cancellation, factory,
                                      root / "output", true, 2, pending));
    returned.store(true, std::memory_order_release);
    execution_returned.Signal();
  });
  probe->second_task_blocked.Wait(
      "second MapUpdate task enters its deterministic block");
  probe->first_failure_reached.Wait(
      "first MapUpdate task reaches its deterministic failure");
  Check(!execution_returned.IsSignaled() &&
  ...
  probe->release_second_task.Signal();
  execution.join();
