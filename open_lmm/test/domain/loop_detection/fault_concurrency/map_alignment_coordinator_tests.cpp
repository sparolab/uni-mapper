...
AlignmentFeedbackSnapshot WaitForSnapshotMatching(
    const std::shared_ptr<AlignmentFeedbackBroker>& broker,
    Predicate predicate, const char* description) {
  struct WaitState {
    std::mutex mutex;
    std::condition_variable condition;
    std::optional<AlignmentFeedbackSnapshot> matched;
  };
  auto state = std::make_shared<WaitState>();
  const auto observe = [state, predicate](
                           const AlignmentFeedbackSnapshot& snapshot) {
    if (!predicate(snapshot)) return;
    {
      std::lock_guard lock(state->mutex);
      state->matched = snapshot;
    }
    state->condition.notify_all();
  };
  broker->SetNotification(observe);
  if (const auto current = broker->Snapshot()) observe(*current);
  {
    std::unique_lock lock(state->mutex);
    const bool ready = state->condition.wait_for(
        lock, test::kWatchdogTimeout,
        [&] { return state->matched.has_value(); });
    Check(ready, description);
  }
  broker->SetNotification({});
  return *state->matched;
}

AlignmentFeedbackSnapshot WaitForSnapshot(
    const std::shared_ptr<AlignmentFeedbackBroker>& broker) {
  return WaitForSnapshotMatching(
      broker, [](const AlignmentFeedbackSnapshot&) { return true; },
      "feedback snapshot notification timeout");
}
...
AlignmentFeedbackSnapshot WaitForAttempt(
  ...
  return WaitForSnapshotMatching(
      broker,
      [=](const AlignmentFeedbackSnapshot& snapshot) {
        return snapshot.session_revision > after_revision &&
               snapshot.attempt_status.state == state;
      },
      "feedback attempt notification timeout");
}

AlignmentFeedbackSnapshot WaitForProposal(
    const std::shared_ptr<AlignmentFeedbackBroker>& broker,
    uint64_t after_revision, AlignmentMethod method) {
  return WaitForSnapshotMatching(
      broker,
      [=](const AlignmentFeedbackSnapshot& snapshot) {
        return snapshot.session_revision > after_revision &&
               snapshot.proposal.method == method;
      },
      "feedback proposal notification timeout");
}
...
void TestKissAcceptDoesNotComputeDescriptor() {
  ...
  snapshot = WaitForProposal(broker, pending_revision,
...
void TestDescriptorFallback() {
  ...
  snapshot = WaitForProposal(broker, snapshot.session_revision,
...
void TestFullFallbackToManual() {
  ...
  snapshot = WaitForProposal(broker, kiss_revision,
                             AlignmentMethod::kDescriptor);
...
void TestRunningAttemptRejectsQueuedResponse() {
  ...
  broker->SetEnabled(true);
  auto input = Input(broker);
  test::PhaseGate proposer_gate;
  input.kiss_proposer = [&] {
    proposer_gate.ArriveAndWait("release running KISS proposer");
    return Proposed(Proposal(AlignmentMethod::kKissMatcher, 3));
  };
  Result<MapAlignmentProposal> result =
      Result<MapAlignmentProposal>::Failure(Error::InvalidArgument("not run"));
...
void TestManualValidationRetries() {
  ...
  snapshot = WaitForAttempt(broker, first_revision,
                            AlignmentAttemptState::kFailedRecoverable);
