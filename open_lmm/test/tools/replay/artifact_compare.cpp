...
std::vector<std::string> FindCloudAgents(const fs::path& directory) {
  std::vector<std::string> agents;
  const std::string prefix = "global_map_", suffix = ".pcd";
  for (const auto& entry : fs::directory_iterator(directory)) {
    const std::string name = entry.path().filename().string();
...
int main(int argc, char** argv) {
  ...
  if (argc < 3 || threshold_count < 0 || threshold_count > 5 ||
    ...
    std::cerr << "usage: open_lmm_artifact_compare BASELINE_DIR CANDIDATE_DIR "
                 "[--pcd|--pcd-only] [--agent-map=A=test1,B=test2] "
  ...
  try {
    ...
    const auto agents =
        compare_poses ? FindAgents(baseline) : FindCloudAgents(baseline);
    if (agents.empty()) {
      std::cerr << "baseline has no comparable artifact files\n";
      return 2;
    }
    ...
    for (const auto& agent : agents) {
      ...
      const bool pose_ok =
          !compare_poses ||
          ComparePoses(baseline / ("optimized_poses_" + agent + ".txt"),
                       candidate /
                           ("optimized_poses_" + candidate_agent + ".txt"),
                       limits);
      bool cloud_ok = true;
      if (compare_pcd) {
