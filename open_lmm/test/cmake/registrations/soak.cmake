...
if(OPEN_LMM_ENABLE_NIGHTLY_SOAK)
  foreach(nightly_family IN ITEMS
      runtime_concurrency transaction_fault visualization resource config_apply)
    if(nightly_family STREQUAL "runtime_concurrency")
      set(nightly_target open_lmm_runtime_concurrency_stress_tests)
      set(nightly_owner RuntimeService)
      set(nightly_seed 179424673)
    elseif(nightly_family STREQUAL "transaction_fault")
      set(nightly_target open_lmm_transaction_fault_stress_tests)
      set(nightly_owner FileSetTransaction)
      set(nightly_seed 15485863)
    elseif(nightly_family STREQUAL "visualization")
      set(nightly_target open_lmm_visualization_stress_tests)
      set(nightly_owner VisualizationProjector)
      set(nightly_seed 32452843)
    elseif(nightly_family STREQUAL "resource")
      set(nightly_target open_lmm_resource_stress_tests)
      set(nightly_owner ResourceGovernor)
      set(nightly_seed 67867967)
    else()
      set(nightly_target open_lmm_config_apply_stress_tests)
      set(nightly_owner ConfigTransaction)
      set(nightly_seed 86028121)
    endif()
    openlmm_add_test(
      NAME open_lmm_${nightly_family}_nightly_soak_tests
      COMMAND $<TARGET_FILE:${nightly_target}>
      COMMAND_ARGS --iterations 1000 --warmup 100 --seed ${nightly_seed}
        --profile nightly
      LAYER L6 MODULE workflows.soak OWNER ${nightly_owner}
  ...
  openlmm_add_test(
    NAME open_lmm_plugin_nightly_soak_tests
    COMMAND $<TARGET_FILE:open_lmm_plugin_stress_tests>
    COMMAND_ARGS
      $<TARGET_FILE:open_lmm_plugin_fixture_valid>
      $<TARGET_FILE:open_lmm_plugin_fixture_wrong_abi>
      $<TARGET_FILE:open_lmm_plugin_fixture_null_factory>
      ...
      $<TARGET_FILE:open_lmm_plugin_fixture_null_entry>
      ${open_lmm_soak_builtin_plugin_arg}
      --iterations 1000 --warmup 100 --seed 49979687 --profile nightly
    LAYER L6 MODULE workflows.soak OWNER PluginLoader
    INVARIANTS INV-12 INV-15 INV-16 INV-17 INV-18
    LANES nightly
    TIMEOUT 5400)
...
if(OPEN_LMM_ENABLE_NIGHTLY_SOAK)
  add_executable(open_lmm_runtime_nightly_soak_tests
    soak/runtime_lifecycle_stress.cpp)
  target_compile_definitions(open_lmm_runtime_nightly_soak_tests PRIVATE
    OPEN_LMM_SOAK_SANITIZER_NAME="${open_lmm_soak_sanitizer_name}")
  target_link_libraries(open_lmm_runtime_nightly_soak_tests PRIVATE
    open_lmm_soak_support
    open_lmm_map_server)
  openlmm_set_global_target_properties(open_lmm_runtime_nightly_soak_tests)
  openlmm_add_test(
    NAME open_lmm_runtime_nightly_soak_tests
    TARGET open_lmm_runtime_nightly_soak_tests
    COMMAND_ARGS --iterations 1000 --warmup 100 --seed 104729
      --profile nightly --scenario all-headless
    LAYER L6 MODULE workflows.soak OWNER RuntimeWorkflow
    INVARIANTS INV-01 INV-02 INV-04 INV-07 INV-08 INV-09 INV-16 INV-17 INV-18
    LANES nightly
    TIMEOUT 5400)
endif()
