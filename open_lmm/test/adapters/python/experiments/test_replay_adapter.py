...
class ReplayAdapterTests(unittest.TestCase):
    ...
    def test_existing_goal03_comparator_contract(self) -> None:
        source = Path(os.environ["OPEN_LMM_SOURCE_ROOT"])
        comparator = Path(os.environ["OPEN_LMM_REPLAY_COMPARE"])
        self.assertTrue(comparator.is_file())
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            template = source / "test/replay/fixtures/tiny_fixture_report_v1.json"
            baseline = source / "test/replay/fixtures/tiny_fixture_baseline_v1.json"
            runner = root / "runner"
            runner.write_text(
