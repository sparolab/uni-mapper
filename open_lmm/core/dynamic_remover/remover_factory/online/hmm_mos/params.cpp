...
HmmMosParams::HmmMosParams() {
  ...
  occupancy_sigma =
      config.param<double>("dynamic_remover", "occupancy_sigma", 0.2);
  free_sigma = config.param<double>("dynamic_remover", "free_sigma", 0.2);
  belief_threshold =
      config.param<double>("dynamic_remover", "belief_threshold", 0.99);
  conv_size = config.param<int>("dynamic_remover", "conv_size", 5);
  local_window_size =
      config.param<int>("dynamic_remover", "local_window_size", 3);
  global_window_size =
      config.param<int>("dynamic_remover", "global_window_size", 300);
