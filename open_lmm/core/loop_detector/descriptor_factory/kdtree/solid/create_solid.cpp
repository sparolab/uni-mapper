...
using SolidV2 = open_lmm::DescriptorPluginV2Dispatcher<SOLiD>;
...
void* Create(const OpenLmmPluginConfigV1* config) noexcept {
  return open_lmm::CreateConfiguredPlugin<IDescriptorKdtree, SOLiD, SolidParams>(
      config, "solid");
}
...
const OpenLmmPluginApiV1 kApi{
    OPEN_LMM_PLUGIN_ABI_VERSION_V1, "descriptor", "solid", &Create, &Destroy,
    "descriptor:kdtree", 1, "open-lmm-1.0"};
...
extern "C" const OpenLmmPluginApiV1* open_lmm_plugin_entry() { return &kApi; }
...
open_lmm_plugin_query_v2(open_lmm_plugin_descriptor_v2* descriptor) {
  ...
  static const open_lmm_artifact_format_v2 formats[] = {
      {sizeof(open_lmm_artifact_format_v2), OPEN_LMM_PLUGIN_ABI_V2_MAJOR,
       OPEN_LMM_PLUGIN_ABI_V2_MINOR, open_lmm::DescriptorV2Text("solid"), 1,
       0, 40}};
  *descriptor = open_lmm::DescriptorV2Header<open_lmm_plugin_descriptor_v2>();
  ...
  descriptor->plugin_name = open_lmm::DescriptorV2Text("solid");
  ...
  descriptor->plugin_id =
      open_lmm::DescriptorV2Text("open_lmm.descriptor.solid");
  ...
  descriptor->schema_id = open_lmm::DescriptorV2Text("descriptor.solid");
...
open_lmm_plugin_open_v2(const open_lmm_host_api_v2* host,
  ...
  *out = nullptr;
  try {
    ...
    auto parsed = open_lmm::Config::FromJson(json, "solid ABI-v2");
    if (!parsed.is_valid()) {
      return open_lmm::DescriptorV2Status(
          OPEN_LMM_STATUS_INVALID_ARGUMENT_V2, "invalid SOLiD JSON");
    }
    auto handle = std::make_unique<open_lmm_plugin_handle_v2>();
    handle->state.host = *host;
    handle->state.model = std::make_unique<SOLiD>(SolidParams(parsed));
    *out = handle.release();
    return open_lmm::DescriptorV2Status(OPEN_LMM_STATUS_OK_V2, "");
  } catch (...) {
    return open_lmm::DescriptorV2Status(
        OPEN_LMM_STATUS_PLUGIN_ERROR_V2,
        "failed to create SOLiD ABI-v2 plugin");
  }
}
...
open_lmm_plugin_call_v2(open_lmm_plugin_handle_v2* handle,
  ...
  return SolidV2::Call(handle ? &handle->state : nullptr, "solid", call, sink);
}
