...
file(READ "${OPEN_LMM_SOURCE_DIR}/../applications/gui/CMakeLists.txt" gui_cmake)
foreach(expected IN ITEMS
  ...
  string(FIND "${gui_cmake}" "${expected}" found)
  if(found EQUAL -1)
    message(FATAL_ERROR "standalone canonical GUI owner is missing: ${expected}")
  endif()
endforeach()
if(EXISTS "${OPEN_LMM_SOURCE_DIR}/src/adapters/gui/CMakeLists.txt")
  message(FATAL_ERROR "legacy core GUI CMake owner must not remain")
endif()
...
foreach(source IN LISTS open_lmm_sources)
  if(source STREQUAL
       "${OPEN_LMM_SOURCE_DIR}/src/foundation/logging/logging.cpp" OR
     source STREQUAL
       "${OPEN_LMM_SOURCE_DIR}/../applications/gui/src/iridescence/application_log.cpp")
    continue()
  endif()
  file(READ "${source}" contents)
...
file(READ "${OPEN_LMM_SOURCE_DIR}/../applications/cli/CMakeLists.txt"
  batch_cmake)
string(FIND "${batch_cmake}" "add_executable(open_lmm_batch src/main.cpp)"
...
foreach(expected
    ...
    "COMPONENTS client"
    "open_lmm::client")
  string(FIND "${batch_cmake}" "${expected}" found)
  if(found EQUAL -1)
    message(FATAL_ERROR
      "standalone batch package must contain: ${expected}")
  endif()
endforeach()
file(READ "${OPEN_LMM_SOURCE_DIR}/../applications/cli/src/main.cpp" batch_main)
...
foreach(forbidden
  ...
  if(NOT found EQUAL -1)
    message(FATAL_ERROR
      "batch launcher must not consume private logging: ${forbidden}")
  endif()
endforeach()
foreach(forbidden
    "open_lmm_utils"
...
foreach(legacy_batch_owner IN ITEMS
    "${OPEN_LMM_SOURCE_DIR}/src/adapters/batch/main.cpp"
    "${OPEN_LMM_SOURCE_DIR}/src/adapters/batch/CMakeLists.txt")
  if(EXISTS "${legacy_batch_owner}")
    message(FATAL_ERROR
      "legacy core Batch owner must not remain: ${legacy_batch_owner}")
  endif()
endforeach()
file(READ "${OPEN_LMM_SOURCE_DIR}/CMakeLists.txt" core_cmake)
foreach(forbidden "applications/cli" "src/adapters/batch" "open_lmm_batch")
  string(FIND "${core_cmake}" "${forbidden}" found)
  if(NOT found EQUAL -1)
    message(FATAL_ERROR
      "core CMake must not reverse-reference CLI owner: ${forbidden}")
  endif()
endforeach()
...
assert_file_excludes(
  ...
  "    LogInfo(profile.str())")
file(READ
  "${OPEN_LMM_SOURCE_DIR}/../applications/gui/src/iridescence/iridescence_gui.cpp"
  iridescence_gui_source)
foreach(expected
    "RecentApplicationLogs"
    "ApplicationLogInfo")
  string(FIND "${iridescence_gui_source}" "${expected}" found)
  if(found EQUAL -1)
    message(FATAL_ERROR
      "Iridescence application-local logging must contain: ${expected}")
  endif()
endforeach()
assert_file_excludes(
...
file(READ
  "${OPEN_LMM_SOURCE_DIR}/../applications/gui/src/iridescence/application_log.cpp"
  iridescence_application_log_source)
foreach(expected
    "spdlog::sinks::ringbuffer_sink_mt"
    "spdlog::info")
  string(FIND "${iridescence_application_log_source}" "${expected}" found)
  if(found EQUAL -1)
    message(FATAL_ERROR
      "Iridescence app-local log boundary must contain: ${expected}")
  endif()
endforeach()
...
file(GLOB_RECURSE experiment_python LIST_DIRECTORIES false
  "${OPEN_LMM_SOURCE_DIR}/../bindings/python/package/open_lmm/experiments/*.py")
